...
def extract_text_node(state: ResumeState) -> ResumeState:
    ...
    try:
        if filename.endswith(".pdf"):
            ...
            state["error"] = "Unsupported file format. Only PDF and DOCX are supported."
            return state
            
        if not text.strip():
            state["error"] = "Could not extract text from the document."
...
def generate_summary_node(state: ResumeState) -> ResumeState:
    ...
    prompt = f"""
    You are an expert technical recruiter. Based on the following candidate resume, 
    extract a short summary in bullet points detailing the strongest points about the candidate and why we should hire them.
    ...
    try:
        ...
        state["parsed_resume_json"] = response.text.strip()
    except Exception as e:
...
def check_scores_node(state: ResumeState) -> ResumeState:
    ...
    text = state.get("resume_text", "")
    prompt = f"""
    You are an expert ATS (Applicant Tracking System) scorer. Score the following resume out of 100 based on the following default criteria:
    ...
    Evaluate the resume honestly and rigorously. 
    ...
    try:
        ...
        if match:
            state["resume_score"] = float(match.group())
        else:
            state["resume_score"] = 0.0
...
def process_resume_with_agent(name: str, email: str, phone: str, file_bytes: bytes, filename: str) -> dict:
