  ...
  return (
    <div className="flex items-center justify-between px-8 py-5 border-b border-line bg-paper sticky top-0 z-50">
      ...
      <div className="flex gap-1.5 items-center">
        ...
        {/* Single "Sign in" button with dropdown — or active-state label if already authed */}
        {isAdminSection ? (
          /* Admin is viewing admin pages — show their label */
          <button
            onClick={() => onNavigate("admin-dashboard")}
            className="px-4 py-2 text-sm rounded-full bg-ink text-white transition"
          >
            {isAdminAuthed ? "Dashboard" : "Admin"}
          </button>
        ) : (
          <div className="relative" ref={menuRef}>
            <button
              onClick={() => setMenuOpen((v) => !v)}
              className={`px-4 py-2 text-sm rounded-full transition flex items-center gap-1.5 ${
                isStudentAuthed
                  ? "bg-primary text-white"
                  : "text-inksoft hover:bg-[#EEEFEC] hover:text-ink"
              }`}
            >
              {isStudentAuthed ? "Account" : "Sign in"}
              <svg
            ...
            {menuOpen && (
              <div className="absolute right-0 top-[calc(100%+8px)] w-52 bg-paper border border-line rounded-xl shadow-lg py-1.5 z-50 animate-fade-in">
                ...
                {isStudentAuthed ? (
                  <>
                    <MenuItem
                      ...
                      onClick={() => { onNavigate("my-applications"); setMenuOpen(false); }}
                    />
                    <MenuItem
                      label="Browse Jobs"
                      sub="View open positions"
                      onClick={() => { onNavigate("portal"); setMenuOpen(false); }}
                    />
                  </>
                ) : (
                  <>
                ...
                <MenuItem
                  label={isAdminAuthed ? "Go to Dashboard" : "Admin Sign in"}
                  sub={isAdminAuthed ? "Recruitment overview" : "HR recruiter access"}
                  onClick={() => {
                    onNavigate(isAdminAuthed ? "admin-dashboard" : "admin-login");
                    setMenuOpen(false);
                  }}
                />
              </div>
